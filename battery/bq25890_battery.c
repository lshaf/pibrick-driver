...
struct bq25890_device {
	...
	int chargesz;
	int soc_ema;	/* smoothed SoC %, -1 = unseeded */
...
/* 1S Li-Po OCV-ish curve, centivolts (380 == 3.80V).
 * Bins are ~5cV wide near the knee to keep jitter < ~4% under
 * ±20mV ADC noise (BQ25895 voltage LSB is 20mV). */
...
const int table_size = sizeof(voltage_to_percent_table) / sizeof(VoltageMap);
static int bq25890_calc_lipo_percentage(int voltage_int) {
	int voltage = voltage_int / 10000;
    if (voltage <= voltage_to_percent_table[0].voltage) return 0;
    if (voltage >= voltage_to_percent_table[table_size - 1].voltage) return 100;
    for (int i = 0; i < table_size - 1; i++) {
        if (voltage >= voltage_to_percent_table[i].voltage && voltage < voltage_to_percent_table[i+1].voltage) {
 ...
 * it so the SoC estimate doesn't chase noise. */
#define BQ_VBAT_EMA_SHIFT	2
...
/* ---- SoC tuning (no bench cal available; adjust these when measured) ----
 * RINT_MOHM:  estimated 1S pack internal resistance (+ wiring), milliohm.
 *             Used to back out the I*R term so we lookup against OCV, not
 *             the charge-inflated terminal voltage. Larger pack/thicker
 *             wires -> smaller value.
 * DISCHG_COMP_UV: the chip can't sense load current, so add a small fixed
 *             IR estimate while discharging (~0.5A * ~100mOhm).
 * SOC_MAX_STEP: cap on how many % the reported SoC may move per read, so a
 *             transient voltage swing (charger unplug) ramps, never snaps. */
#define BQ_RINT_MOHM		120
#define BQ_DISCHG_COMP_UV	50000
#define BQ_SOC_MAX_STEP		3
/* Estimated average load current while on battery, uA. The charger IC can't
 * measure discharge current, so this feeds userspace's time-to-empty calc.
 * Default 1A: 5Ah pack / 1A ~= 5h (matches the time_to_empty_avg hardcode). */
#define BQ_DISCHG_CURRENT_UA	1000000

static int bq25890_get_batp(struct power_supply *psy){
	struct bq25890_device *bq = power_supply_get_drvdata(psy);
	int voltage = bq25890_get_batv(psy);
	int ocv, raw, ichgr;

	if (voltage < 0)
		return voltage;

	/* Estimate open-circuit voltage by removing the IR component before
	 * the percentage lookup. Charge current (F_ICHGR) flows into the
	 * battery and inflates terminal V by I*Rint, so subtract it. This is
	 * what caused the big jump on unplug: terminal V was read directly. */
	switch (bq->state.chrg_status) {
	case STATUS_PRE_CHARGING:
	case STATUS_FAST_CHARGING:
		ichgr = bq25890_field_read(bq, F_ICHGR);	/* units of 50mA */
		if (ichgr < 0)
			ichgr = 0;
		/* drop_uV = I(uA)*R(ohm) = (ichgr*50000uA)*(RINT/1000 ohm)
		 *         = ichgr * 50 * RINT_MOHM */
		ocv = voltage - ichgr * 50 * BQ_RINT_MOHM;
		break;
	case STATUS_TERMINATION_DONE:
		ocv = voltage;			/* current ~0: terminal == OCV */
		break;
	default:				/* NOT_CHARGING / discharging */
		ocv = voltage + BQ_DISCHG_COMP_UV;
		break;
	}

	raw = bq25890_calc_lipo_percentage(ocv);

	/* Rate-limit reported SoC so it converges smoothly to the estimate. */
	if (bq->soc_ema < 0)
		bq->soc_ema = raw;		/* first read: seed */
	else if (raw > bq->soc_ema)
		bq->soc_ema += min(raw - bq->soc_ema, BQ_SOC_MAX_STEP);
	else
		bq->soc_ema -= min(bq->soc_ema - raw, BQ_SOC_MAX_STEP);
 ...
 * is cheap when idle.
 */
#define BQ_POLL_INTERVAL_MS	5000
...
static const struct power_supply_desc bq25890_power_supply_desc = {
	// .name = "battery",
	.type = POWER_SUPPLY_TYPE_BATTERY, // POWER_SUPPLY_TYPE_USB,
...
static int bq25890_power_supply_init(struct bq25890_device *bq)
{
	...
	snprintf(bq->name, sizeof(bq->name), "battery");// "bq25890-charger-%d", bq->id);
